...
GEMINI_API_KEY = ""  # IMPORTANT: Replace with your actual Gemini API key
try:
...
def fetch_films_table() -> pd.DataFrame:
    ...
    if resp.status_code != 200:
        raise HTTPException(status_code=502, detail="Failed to fetch Wikipedia")
...
def make_scatterplot_data_uri(df: pd.DataFrame) -> str:
    if not {"Rank","Peak"}.issubset(df.columns): return ""
...
async def analyze(request: Request) -> List[Dict[str, str]]:
    ...
    if "wikipedia.org/wiki/list_of_highest-grossing_films" in text_lower or "highest grossing films" in text_lower:
        try:
            ...
            return [{"question": q, "answer": f"Error fetching data: {e}"} for q in questions]

    if not GEMINI_MODEL:
        return [{"question": q, "answer": "Error: Gemini not available. Please configure GEMINI_API_KEY."} for q in questions]

    # Handle scatterplot question separately as it requires special function call
    answers = []
    other_questions = []
    for q in questions:
...
"""
        async def gemini_call():
            ...
            if m:
                ...
                # Ensure parsed response aligns with questions
                parsed_answers = []
                for i, q in enumerate(other_questions):
                    ...
                    if i < len(parsed) and isinstance(parsed[i], dict) and "answer" in parsed[i]:
                        answer = str(parsed[i]["answer"])
                    parsed_answers.append({"question": q, "answer": answer})
                return parsed_answers
            return [{"question": q, "answer": "Error: Invalid Gemini response"} for q in other_questions]

        gemini_answers = await gemini_call()
        answers.extend(gemini_answers)

    except Exception as e:
        answers.extend([{"question": q, "answer": "Error: " + str(e)} for q in other_questions])
